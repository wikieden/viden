...
### C7 `runtime.durable_work_evidence`（评审通过，2026-09-12）

- **`EvidenceCanonicalized` 保持已发布的形状 `{ evidence_id, item_id,
  content_sha256 }`。** 改动一个实时事件的载荷是破坏性线上变更；完整引用在它紧随的
  `EvidenceRecorded` 行上。
- **审计结果为 `Success` 而非 `Applied`**，枚举中没有后者；`Denied` 表示策略拒绝，
  而操作者的拒绝是一项已被执行的决定。
- **审计对象为权限请求 id、工具名与作业 id，而非工具调用 id。** 审批路径上不存在
  工具调用 id；作业 id 才是与该决定所释放工作的真实关联。
- **`producer.task_id` 在回合绑定任务时取 owner 的任务，否则取回合 id，再否则取
  工具调用 id。** 字面上的回合 id 永远无法满足以任务为键的合并闸门；会话作用域的
  回合仍然不指名任务，以 `MissingProducer` 被拒绝。
- **本批次中 ACP 规范化仅在实时流中生效**（兼容性后续项 10）。原生路径已完全持久化。
- **`bundle_id`** 对原生行取回合最后一个上下文包，对 agent 行取存储句柄 id；从不臆造。
- **受监督回合现在持久化与命令路径相同的持久集合**，包括上下文投影与
