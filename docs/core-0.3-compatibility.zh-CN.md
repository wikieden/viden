...
- `ConflictHunkReason` 由 Core 依据应用所见在一处分类：`ContextMismatch`、文件已包含该 hunk
  ...
  以及 `Binary`。自 H2（2026-09-12）起，本地应用路径会产生 `Binary`：二进制文件没有可供
  拒绝的行，因此文件本身就是那次拒绝，并以一条不带行的 hunk 发布，`base` 为 `None` ——
  「完全没有原像」，与新建文件空区域所携带的 `Some(vec![])` 编码方式不同。客户端永不解析
  消息来决定提供什么操作。
...
1. **严格 apply 会静默丢弃补丁中的二进制文件**（既有问题，C3 期间发现）。
   **已于 2026-09-12 由 H2 关闭。** `parse_unified_diff` 丢弃了所有扫描不到行的
   片段，也就是 Git 报告的每一个二进制文件，因此混合补丁会应用其中的文本文件，
   而对二进制文件只字不提 —— 操作者读到「已应用」，却没有任何途径得知变更的一
   部分从未被写入。这也是 `ConflictHunkReason::Binary` 一直没有生产者的原因。
   现在结果按文件明示，且沿用类型已建模的形状：二进制片段被保留，路径照样校验，
   并作为拒绝而不是变更随准备好的应用一起传递，因此文本文件仍会应用，而 `check`
   与 `apply` 都会在 `PatchApplyOutcome.conflicts` 中点名该二进制文件；若补丁的
   每一个文件都被拒绝，则报告 `applied: false`。`conflict_content` 把它发布为一条
   不带行的 hunk，`reason` 为 `ConflictHunkReason::Binary`，`base` 为 `None`。
   非二进制且没有行的片段 —— Git 仅带头部的模式变更 —— 仍然被丢弃，因为它不携带
   本 apply 能够作出或拒绝的任何变更。没有新增事件、能力或 fixture，九个冻结基线
   fixture 逐字节不变。
2. **D10 的 `eventsUnavailable` 文案混淆了两种状态**（既有问题，H1 期间发现）。
   **已于 2026-09-12 由 H2 关闭。** 原先一条字串说 Core 未发布审计时间线，于是
   一个还没有人加载过的页面也被读成了这句话。现在两者是两种状态、两套文案，双语
   均已提供：「尚未读取」是页面自身的状态，对 Core 什么都不声称；「未提供」以能力
   为条件，并点名 `runtime.audit`，因此这个缺口可以对照 Core 的握手来核对。Core
   未给出措辞的拒绝会回落到读取自身的状态，而不是能力那句话。截图为
