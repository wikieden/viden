...
E1（发布证据：一个从接入到提交变更的真实任务）待办，是退出条件的剩余关卡。
它同时负责 Core `0.3.6` 不可变 checkpoint 的声明与
`crates/core/release-manifest.toml` 中 `component_version` 的提升——迄今没有
任何批次被授权做这两件事。

相对上文基线，有两处契约事实发生变化：GUI-CORE-012、015、020、025 已完全关闭
（Core 已发布，两个客户端均已采纳）；GUI-CORE-027（工作区级操作者身份）已开立
并排入 `0.3.4`。当前开放的登记项为 009、013、018、019、021、023、026、027。
