...
- The command's `owner` equals its envelope owner, which is what Core's
  supervisor requires. For a Lane target it is the runtime owner Core published
  for that exact Lane; otherwise it is this client's default envelope owner.
...
with `--provider fallback --model test-local` in tmux. Captured:
