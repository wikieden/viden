...
`ReadWorkspaceFile` 会在 `read_file` 权限闸门之后回答单个文件的字节。没有任何登记项
因此关闭：它是作为 Files 页签、Code 页签与命令面板文件行的契约内容排期的，而不是
一条编号请求；GUI 在 G7 中采纳。当前开放的登记项不变，仍为 009、013、018、019、
021、023、026、028。
...
**2026-09-12 修订（`0.3.4`，批次 G6）。** D14 现在把两组 chip 作为明确的
*已加载页*筛选提供，而不再是完全不提供。本条目要保护的性质并未改变，也正是这
一点让改动成立：筛选条上的每个标签都点明已加载页，结果分布条的标题是
`已加载页的结果分布` 而绝不是总数，筛到空时显示「已加载页中没有符合这些筛选条
件的记录」——这与「Core 未为该视图记录任何审计条目」是不同的一句话。此处没有
任何地方声称完整性，因此本条目警告的那种误报无法发生。请求本身维持原文：只有在
分页之前应用的服务端过滤器，才能让这些 chip 描述整条时间线而不是一页；设计稿画
在它们旁边的导出控件，在命令出现之前保持禁用并注明原因。
...
"查看证据…"行以及 `/evidence` 系统命令打开。参见
`docs/release-tui-0.3.4-source-control-parity.zh-CN.md`。
