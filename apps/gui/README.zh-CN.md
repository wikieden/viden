...
每个路由槽位——包括 tooltip 与可访问名称——都以它实际打开的屏幕命名：集成闸（D12）、
决策（D2）、审计时间线（D14）、Lane 监视墙（D10）、舰队看板（D13），每个都使用已登记
`GUI/gui-icons.jsx` 中与该屏幕最接近的字形。此前 rail 在这同一组路由上套用的是编辑器的
文件浏览器词汇（搜索、源代码管理、证据、诊断、收件箱），这比没有标签更糟：操作者只能靠
点击才能学到真实映射，在此之前每一条 tooltip 与读屏播报都在教错的东西。这次改动之后
目的地是诚实的，路由本身没有变化。当时仍然悬而未决的是**设计**问题而非标注问题——被接受的
rail 中并没有决策队列、审计轨迹、Lane 监视墙或舰队看板的槽位，设计里其中几个是以驾驶舱
内的次级视图抵达的，因此这四个槽位曾是一个等待设计裁定的路由决定。该裁定此后已经落定：
设计包 `docs/SPEC.md` 的决策 `D-RAILNAV`（2026-09-08）接受 rail 作为通往独立 D 屏的路由器，
并把旗舰原型的页内视图切换保留为设计探索，因此这里已交付的路由就是被接受的模型，
而不是一个临时方案。
