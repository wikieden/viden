...
H2 批次（2026-09-12）关闭了 E1 的四项卫生缺陷与 D10 的文案待办，其中三项改变了界面
上显示的内容。

**EvidenceView：当初记录的说法，与 Core 刚刚做的那次读取。** 报告中的
`verification` 行携带的是**归档**在写下规范引用时记录的判定；内容答案则是 Core
**此刻**读取那些字节所得到的结果。当后者与前者矛盾时 —— 记录为 `verified`，而内容为
`Unavailable { HashMismatch }` —— 界面用一句话把两者联系起来，并给那条记录加上设计中
的不一致处理（错误色加删除线，因此状态不只靠颜色表达）。那个记录下来的词绝不会被
改写或删除：它就是归档所声称的内容，隐藏它只是同一种不诚实的另一半。`failed` 与
`HashMismatch` 是两个彼此一致的事实，不会产生告警；为另一行回显的内容答案也不构成
矛盾。这一段取代上文关于 GUI-CORE-025 的实现说明：宿主投影确实携带 `verification`
与 `quality`，报告也会把两者都写出来。

**Welcome：尚未打开项目不是 adapter 故障。** Welcome 只在没有绑定工作区时渲染，
因此那里的最近工作读取失败，原因就是还没有打开任何东西。提示以 D1 自己的「尚未打开
项目」开头并说明下一步，宿主原本那句话作为诊断保留在下方 —— 两个事实都不隐藏。若
`runtime.recent_work` 能力缺失，则仍用它自己的措辞，因为那无论是否打开项目都是关于
Core 的事实。`Core adapter is not connected` 只在已绑定工作区的界面上继续作为首句，
那里由 D6 连接状态负责说明。

**Welcome 填满窗口。** 未绑定项目的中间栏是「填充轨道」而不是「百分比」链路：
`.d1-main-welcome` 在 `.d1-main` 之后声明单个 `1fr` 轨道，从而赢得它原先输掉的同权重
之争，工作面则是单轨 grid，Welcome 作为 grid 项被拉伸填满。全程不涉及任何像素高度，
因此布局在任何窗口高度下都一致 —— 而 E1 缺陷 8 正是：无论窗口多高，内容与状态栏都
大约停在 525 px。

**D10 的事件流有两种缺失，而不是一种。**「The Core audit timeline has not been read
yet」是页面自身的状态，对 Core 什么都不声称；「Core 握手未声明 `runtime.audit` 能力」
以能力为条件并点名它，因此这个缺口可以对照握手来核对。Core 未给出措辞的拒绝会回落到
读取自身的状态，而不是能力那句话。

**只有一个座舱持有窗口级快捷键。** `renderD1Cockpit` 把 `⌘K`、`⌘L`、`⌘.`、`⌘G`、
`⌘E`、`⌘R`、`⌘O` 与 `Escape` 注册在 `window` 上并一直持有到被 dispose，因此在另一个
座舱之上挂载座舱会留下两套监听。`bootstrapShell` 丢弃了自己的控制器，于是水合前的
那层外壳在活动座舱下面继续响应这些快捷键，并把它自己的 `connecting` 投影重新渲染进
同一个根节点 —— 这就是报告中项目 chip 退回 `—`、`Core connection pending` 重新出现的
那次回退。现在每一次挂载都经过 `src/main.ts` 中的 `claimRoot`，它会先 dispose 上一个
持有者。
